...
void read_assetDataJson(VulkanEngine* engine, const char* path)
{
    ...
    const Value& data_set = doc["Model_Data"];
    if (!data_set.IsArray())
    {
        printf("Failed to load %s", path);
        return;
    ...
    for (size_t i = 0; i < data_set.Size(); ++i)
    {
        ...
        std::string path = data_set[i]["Path"].GetString();
        
        auto structureFile = loadGltf(engine, path);

        assert(structureFile.has_value());

        engine->loadedGLTFs[m_id] = *structureFile;
    }
...
void read_hitboxJson(VulkanEngine* engine, const char* path)
{
    ...
    const Value& data_set = doc["Hitbox_Data"];
    if (!data_set.IsArray())
    {
        printf("Failed to load %s", path);
        return;
    ...
    {
        std::string m_id = data_set[i]["Model_ID"].GetString();
        int hbCount = data_set[i]["Collider_Num"].GetInt();

        for (int x = 0; x < hbCount; ++x)
        {
...

{
    ...
    for (int x = 8; x < engine->_vGameObjects.size(); ++x)
    {
        ...
        Pwriter.Key("Name");
        Pwriter.String(GO.Name.c_str());

        Pwriter.Key("Position_x");
        Pwriter.Double((double)GO._transform.position.x);
        Pwriter.Key("Position_y");
        Pwriter.Double((double)GO._transform.position.y);
...
bool write_hitboxJson(VulkanEngine* engine, std::string path)
{
    ...
    for (int x = 8; x < engine->_vGameObjects.size(); ++x)
    {
        ...
        for (int x = 0; x < hbCount; ++x)
        {
            auto& obb = GO._vOBB[x];
            std::string s = "Extent_";
            s += std::to_string(x) += "_x";     // Extents x
            Pwriter.Key(s.c_str());
            Pwriter.Double(obb.e.x);
          

            s = "Extent_";
            s += std::to_string(x) += "_y";    // Extents y
            Pwriter.Key(s.c_str());
            Pwriter.Double(obb.e.y);
         
            s = "Extent_";
            s += std::to_string(x) += "_z";    // Extents z
            Pwriter.Key(s.c_str());
            Pwriter.Double(obb.e.z);

            s = "Centre_";
            s += std::to_string(x) += "_x";     // Centre_ x
            Pwriter.Key(s.c_str());
            Pwriter.Double(obb.c.x);


            s = "Centre_";
            s += std::to_string(x) += "_y";    // Centre_ y
            Pwriter.Key(s.c_str());
            Pwriter.Double(obb.c.y);

            s = "Centre_";
            s += std::to_string(x) += "_z";    // Centre_ z
            Pwriter.Key(s.c_str());
            Pwriter.Double(obb.c.z);
        }
